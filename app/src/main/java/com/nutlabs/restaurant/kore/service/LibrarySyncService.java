 ...
 */
package com.nutlabs.restaurant.kore.service;
...
import com.nutlabs.restaurant.kore.jsonrpc.event.MediaSyncEvent;
import com.nutlabs.restaurant.kore.jsonrpc.method.AudioLibrary;
import com.nutlabs.restaurant.kore.jsonrpc.method.VideoLibrary;
import com.nutlabs.restaurant.kore.jsonrpc.type.AudioType;
import com.nutlabs.restaurant.kore.jsonrpc.type.LibraryType;
import com.nutlabs.restaurant.kore.jsonrpc.type.ListType;
import com.nutlabs.restaurant.kore.jsonrpc.type.VideoType;
...
public class LibrarySyncService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (syncAllMovies) {
            syncOrchestrator.addSyncItem(new SyncMovies(hostInfo.getId(), syncExtras,pref));
        }
        ...
        if (syncSingleMovie) {
            ...
            if (movieId != -1) {
                syncOrchestrator.addSyncItem(new SyncMovies(hostInfo.getId(), movieId, syncExtras,pref));
            }
        }
    ...
    public interface SyncItem {
         ...
         * @return Sync type, one of the constants in {@link com.nutlabs.restaurant.kore.service.LibrarySyncService}
         */
    ...
    private static class SyncMovies implements SyncItem {
         ...
         */
        public SyncMovies(final int hostId, Bundle syncExtras, SharedPreferences pref) {
            this.hostId = hostId;
            this.movieId = -1;
         ...
         * @param hostId XBMC host id
         */
        public SyncMovies(final int hostId, final int movieId, Bundle syncExtras, SharedPreferences pref) {
            this.hostId = hostId;
        ...
        public void sync(final SyncOrchestrator orchestrator,
            ...
            String properties[] = {
                    ...
                    VideoType.FieldsMovie.SETID,VideoType.FieldsMovie.TAG,
                    //jVideoType.FieldsMovie.GENREID
            ...
            action.execute(hostConnection, new ApiCallback<List<VideoType.DetailsMovie>>() {
                ...
                public void onSuccess(List<VideoType.DetailsMovie> result) {
                    ...
                    if (result.size() == LIMIT_SYNC_MOVIES) {
                        ...
                        chainCallSyncVideocGenres(orchestrator, hostConnection, callbackHandler, contentResolver);
                        orchestrator.syncItemFinished();
                    }
                }
        ...
        private void insertMovies(final SyncOrchestrator orchestrator,
            ...
            String genreShortMovies="",genreAnimatedMovies="",genreTedTalks="",genreOldIsGold="",genreViralVideos="",genreNewAddition="";
            String prefTagString="";
            ArrayList<String> lstTag1=new ArrayList<String>();
            ArrayList<String> lstTag2=new ArrayList<String>();
            lstTag1.add("computerskills,Basic,Chap1");
            lstTag2.add("computerskills,Advance,Chap1");
            ...
            for (int i = 0; i < movies.size(); i++) {
                ...
                for(String tag:movie.tag){
                    if (tag.compareToIgnoreCase(SharedConstants.MOVIE_TAG_SHORTMOVIES)==0){
                        //delme changing the tag for testing
                        /*movieValuesBatch[i].put("tag","computerskills,basic,chap1");
                        if(!prefTagString.contains("computerskills,basic,chap1")){
                            prefTagString+="computerskills,basic,chap1"+"#";
                        }
                        */

                        for(String genre:movie.genre){
                            if(!genreShortMovies.contains(genre)){
                                genreShortMovies+=genre+",";
                            }
                        }
                    }else if(tag.compareToIgnoreCase(SharedConstants.MOVIE_TAG_ANIMATEDMOVIE)==0){
                       /* movieValuesBatch[i].put("tag","computerskills,Advance,Chap1");
                        if(!prefTagString.contains("computerskills,Advance,Chap1")){
                            prefTagString+="computerskills,Advance,Chap1"+"#";
                        }
                        */
                        for(String genre:movie.genre){
                            if(!genreAnimatedMovies.contains(genre)){
                                genreAnimatedMovies+=genre+",";
                            }
                        }
                    }else if(tag.compareToIgnoreCase(SharedConstants.MOVIE_TAG_TEDTALKS)==0){
                        for(String genre:movie.genre){
                            if(!genreTedTalks.contains(genre)){
                                genreTedTalks+=genre+",";
                            }
                        }
                    }
                    else if(tag.compareToIgnoreCase(SharedConstants.MOVIE_TAG_OLDISGOLD)==0){
                        for(String genre:movie.genre){
                            if(!genreOldIsGold.contains(genre)){
                                genreOldIsGold+=genre+",";
                            }
                        }
                    }
                    else if(tag.compareToIgnoreCase(SharedConstants.MOVIE_TAG_VIRAL_VIDEOS)==0){
                        for(String genre:movie.genre){
                            if(!genreViralVideos.contains(genre)){
                                genreViralVideos+=genre+",";
                            }
                        }
                    }
                    else if(tag.compareToIgnoreCase(SharedConstants.MOVIE_TAG_NEW_ADDITION)==0){
                        for(String genre:movie.genre){
                            if(!genreNewAddition.contains(genre)){
                                genreNewAddition+=genre+",";
                            }
                        }
                    }
                    else if(tag.contains(",")){
                        if(!prefTagString.contains(tag)){
                            prefTagString+=tag+"#";
                        }
                    }
                }
            ...
            editor.putString(SharedConstants.PREF_SHORTMOVIES,genreShortMovies);
            editor.putString(SharedConstants.PREF_ANIMATEDMOVIE,genreAnimatedMovies);
            editor.putString(SharedConstants.PREF_TEDTALKS,genreTedTalks);
            editor.putString(SharedConstants.PREF_OLDISGOLD,genreOldIsGold);
            editor.putString(SharedConstants.PREF_VIRALVIDEOS,genreViralVideos);
            editor.putString(SharedConstants.PREF_NEWADDITIONS,genreNewAddition);
            editor.putString(SharedConstants.PREF_FOLDERSTRUCTURE,prefTagString);
            editor.commit();
         ...
         * Genres->Albums->Songs
         */
        private void chainCallSyncVideocGenres(final SyncOrchestrator orchestrator,
                                               final HostConnection hostConnection,
                                               final Handler callbackHandler,
                                               final ContentResolver contentResolver) {
            // Genres->Albums->Songs
            VideoLibrary.GetGenres action = new VideoLibrary.GetGenres("movie",getGenresProperties);
            ...
            action.execute(hostConnection, new ApiCallback<List<LibraryType.DetailsGenre>>() {
                ...
                public void onSuccess(List<LibraryType.DetailsGenre> result) {
                    ...
                    for (int i = 0; i < result.size(); i++) {
                        ...
                        genresValuesBatch[i] = SyncUtils.contentValuesFromVideoGenre(hostId, genre);
                    }

                    // Insert the genres and proceed to albums
                    contentResolver.bulkInsert(MediaContract.VideoGenres.CONTENT_URI, genresValuesBatch);
    ...
    private static class SyncTVShows implements SyncItem {
         ...
         * in a sequential manner (defeating the parallel nature of host calls)1
         * @param orchestrator Orchestrator to call when finished
         * @param hostConnection Host connection to use
         * @param callbackHandler Handler on which to post callbacks
         * @param contentResolver Content resolver
    ...
    private static class SyncMusic implements SyncItem {
        ...
        private void chainCallSyncGenres(final SyncOrchestrator orchestrator,
                                         ...
                                         final ContentResolver contentResolver) {
            // Genres->Albums->Songs
            //AudioLibrary.GetGenres action = new AudioLibrary.GetGenres("movie",getGenresProperties);
            AudioLibrary.GetGenres action = new AudioLibrary.GetGenres(getGenresProperties);
            action.execute(hostConnection, new ApiCallback<List<LibraryType.DetailsGenre>>() {
                @Override
                public void onSuccess(List<LibraryType.DetailsGenre> result) {
